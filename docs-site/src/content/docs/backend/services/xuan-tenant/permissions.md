...
| 权限码 | 动作 | 说明 |
| --- | --- | --- |
| `tenant:view` | 查询 | 访问列表、详情和下拉引用数据 |
| `tenant:create` | 新增 | 创建业务数据 |
| `tenant:update` | 修改 | 修改业务数据 |
| `tenant:delete` | 删除 | 删除、作废或停用业务数据 |
| `tenant:audit` | 审核 | 审核、反审核、红冲等强业务动作，按需启用 |
| `tenant:export` | 导出 | 导出列表或明细数据，按需启用 |

## 列权限

| pageKey | 字段范围 | 说明 |
| --- | --- | --- |
| `tenant` | 租户联系人、手机号、合同信息等敏感列 | 具体字段开发时按页面补齐 |
