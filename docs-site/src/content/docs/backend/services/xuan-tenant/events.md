...
| --- | --- | --- | --- | --- |
| TenantCreated, TenantDisabled, TenantEnabled, TenantPlanChanged | `xuan-tenant-event` | 租户创建、启停、套餐变更事件 | 按业务协作需要订阅 | 事件结构开发时补齐 |

## 订阅事件

| 来源服务 | 事件 | 处理目的 | 幂等键 |
| --- | --- | --- | --- |
| 按协作关系确定 | IAM 租户初始化完成事件 | 构建本服务状态或读模型 | `eventId` |
