-- Synchronize tenant_admin permissions and tenant menus to the selected IAM init template.
-- V1-V19 history remains unchanged; this migration only replaces the main bootstrap function.
...
BEGIN
    ...
    WHERE role_permission.permission_id = permission.id
      AND role_permission.tenant_id = p_tenant_id
      ...
      AND NOT EXISTS (
          SELECT 1
          FROM selected_permission
          WHERE selected_permission.code = permission.code
      );
...
COMMENT ON FUNCTION bootstrap_iam_tenant(bigint, varchar, varchar, varchar, varchar, varchar, varchar, varchar)
    IS '按初始化模板同步 IAM 租户菜单、tenant_admin 角色权限、管理员授权快照和 outbox 事件；模板切换时会收敛旧模板多余授权。';
