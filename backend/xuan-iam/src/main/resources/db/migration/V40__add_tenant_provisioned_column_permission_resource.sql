-- 补齐租户管理页“开通完成”字段资源，使列权限配置器覆盖真实表格的全部展示列。
...
SELECT 'tenant', 'provisionedAt', '开通完成', 'DATETIME', NULL, true, 90, 'system-v40', 'system-v40'
WHERE NOT EXISTS (
    SELECT 1
    FROM iam_resource_column existing
    WHERE existing.resource_key = 'tenant'
      AND existing.column_key = 'provisionedAt'
      AND existing.deleted_at IS NULL
);

WITH template_items(template_code, column_key, access_mode) AS (
    VALUES
    ('tenant_admin_default', 'provisionedAt', 'VISIBLE'),
    ('tenant_readonly_masked', 'provisionedAt', 'VISIBLE')
)
INSERT INTO iam_column_permission_template_item (
    template_id, resource_column_id, access_mode, created_by, updated_by
)
SELECT template.id, resource_column.id, template_items.access_mode, 'system-v40', 'system-v40'
FROM template_items
