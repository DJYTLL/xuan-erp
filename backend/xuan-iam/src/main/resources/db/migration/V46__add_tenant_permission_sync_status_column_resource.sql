-- 补齐租户管理页“权限同步状态”字段资源，使 tenant 列权限草稿、前端 schema 和 IAM 列资源保持一致。
...
SELECT 'tenant', 'permissionSyncStatus', '权限同步状态', 'ENUM', NULL, true, 95, 'system-v46', 'system-v46'
WHERE NOT EXISTS (
    SELECT 1
    FROM iam_resource_column existing
    WHERE existing.resource_key = 'tenant'
      AND existing.column_key = 'permissionSyncStatus'
      AND existing.deleted_at IS NULL
);

WITH template_items(template_code, column_key, access_mode) AS (
    VALUES
    ('tenant_admin_default', 'permissionSyncStatus', 'VISIBLE'),
    ('tenant_readonly_masked', 'permissionSyncStatus', 'VISIBLE')
)
INSERT INTO iam_column_permission_template_item (
    template_id, resource_column_id, access_mode, created_by, updated_by
)
SELECT template.id, resource_column.id, template_items.access_mode, 'system-v46', 'system-v46'
FROM template_items
